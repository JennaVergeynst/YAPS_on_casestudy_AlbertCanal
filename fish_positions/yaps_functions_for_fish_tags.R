...
prepare_and_run <- function(toa_list_el, hydro_data, pingType, X_shift, Y_shift, results_path, time_col='synced_time', min_obs=3){
  library(dplyr)
  ...
  write.csv(residuals_df,paste0(results_path,'/residuals_',toString(ID),'_pass_', toString(groups_pas),'.csv'))
  
  if (nrow(track)==0){
    DATETIME <- toa_info[,time_col]
    write.csv(DATETIME, paste0(results_path, '/failed_period_',toString(ID),'_pass_', toString(groups_pas), '.csv'))
  }
...
runTrack <- function(toa, hydros, pingType, rbi_min, rbi_max, X_shift, Y_shift, ss_data){
  ...
  if (mean(est_error)>100){ # return empty dataframe if estimated error is too large
    track <- data.frame()
  }
